...
def solve(colors, cost, time_limit=300, logfile="gurobi.log", threads=1):
    ...
    # is_tangentially_connected = {}
    is_tangentially_connected = {}
    for i in range(N):
        for j in range(i + 1, N):
            if (i, j) in is_edge_activated:
                for k in range(N):
                    if (j, k) in is_edge_activated and (i, k) in is_edge_activated:
                        is_tangentially_connected[i, j, k] = model.addVar(
                            vtype=GRB.BINARY, name=f"is_tangentially_connected_{i}_{j}_{k}"
                        )

    for i, j, k in is_tangentially_connected:
        model.addConstr(is_tangentially_connected[i, j, k] * 2 <= is_edge_activated[i, k] + is_edge_activated[j, k])
    ...
    model.setObjective(
        ...
        ) + gp.quicksum(cost[i, j] * is_tangentially_connected[i, j, k] for i, j, k in is_tangentially_connected),
        GRB.MAXIMIZE,
    )
    ...
    for i, component in enumerate(components):
        ...
        for j in component:
            ...
            print(
                f"Active outgoing edges: {[(j, k) for k in range(N) if (j, k) in is_edge_activated and is_edge_activated[j, k].X > 0.5]}")
            print(
                f"Active incoming edges: {[(k, j) for k in range(N) if (k, j) in is_edge_activated and is_edge_activated[k, j].X > 0.5]}")
